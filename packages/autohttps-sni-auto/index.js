...
module.exports.create = function(autoSni) {
  if (!autoSni.getCertificatesAsync) {
    autoSni.getCertificatesAsync = require('bluebird').promisify(
      autoSni.getCertificates
    );
  }
  ...
  var _autoSni = {
    ...
    cacheCerts: function(certs) {
      var meta = {
        certs: certs,
        tlsContext: ('string' === typeof certs.cert &&
        ...
        renewAt: certs.expiresAt -
          (autoSni.renewWithin - autoSni._renewWindow * Math.random()),
    ...
    sniCallback: function(domain, cb) {
      ...
      if (!certMeta) {
        ...
        promise = autoSni
          .getCertificatesAsync(domain, certMeta.certs)
          .then(autoSni.cacheCerts);
        ...
        if (certMeta.auto && now >= certMeta.renewAt) {
          ...
          certMeta.renewAt = (autoSni._dbg_now || Date.now()) +
            2 * MIN +
            3 * MIN * Math.random();
      ...
      promise.then(
        ...
        function(err) {
          ...
          delete autoSni._ipc[
            certMeta && certMeta.subject ? certMeta.subject : domain
          ];
        }
      );
    },
