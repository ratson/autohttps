...
var tests = [
  ...
  function(/*domain, certs, cb*/) {
    ...
    throw new Error(
      'Should not call register renew a certificate with more than 10 days left'
    );
  },
  function(domain, certs, cb) {
    count += 1;
          ...
          leSni.sniCallback('example.com', function(err, tlsContext) {
            ...
            leSni.sniCallback('example.com', function(err, tlsContext) {
              ...
              if (expectedCount !== count) {
                throw new Error(
                  'getCertificate only called ' + count + ' times'
                );
              }

              if (expectedShared !== shared) {
                throw new Error(
                  'wrongly used only ' + shared + ' shared promises'
                );
              }
