  ...
  function registerNewAccount(options, cb) {
    ...
    function getTerms(err, res) {
      ...
      if (Math.floor(res.statusCode / 100) !== 2) {
        return handleErr(
          err,
          'Registration request failed: ' + res.body.toString('utf8')
        );
      }
      ...
      if (!links || !('next' in links)) {
        return handleErr(
          err,
          "Server didn't provide information to proceed (1)"
        );
      }
      ...
      if (state.termsRequired) {
        ...
        options.agreeToTerms(state.termsUrl, function(err, agree) {
          ...
          if (!agree) {
            return handleErr(
              new Error(
                "You must agree to the terms of use at '" + state.termsUrl + "'"
              )
            );
          }
    ...
    function sendAgreement() {
      if (state.termsRequired && !state.agreeTerms) {
        return handleErr(
          null,
          'The CA requires your agreement to terms: ' + state.termsUrl
        );
      }
      ...
      state.acme.post(
        ...
        function(err, res, body) {
          ...
          if (err || Math.floor(res.statusCode / 100) !== 2) {
            return handleErr(
              err,
              "Couldn't POST agreement back to server",
              body
            );
          }
    ...
    if (!options.agreeToTerms) {
      cb(
        new Error(
          'options.agreeToTerms must be function (tosUrl, fn => (err, true))'
        )
      );
      return;
    }
