  ...
  function getCert(options, cb) {
    ...
    function getReadyToValidate(err, res, body) {
      ...
      if (Math.floor(res.statusCode / 100) !== 2) {
        return handleErr(
          null,
          'Authorization request failed (' + res.statusCode + ')'
        );
      }
      ...
      if (!links || !('next' in links)) {
        return handleErr(
          err,
          "Server didn't provide information to proceed (2)"
        );
      }
      ...
      if (httpChallenges.length === 0) {
        return handleErr(
          null,
          "Server didn't offer any challenge we can handle."
        );
      }
    ...
    function ensureValidation(err, res, body, unlink) {
      ...
      if (err || Math.floor(res.statusCode / 100) !== 2) {
        ...
        return handleErr(
          err,
          'Authorization status request failed (' +
      ...
      if (authz.status === 'pending') {
        ...
        unlink();
        challengesState = (authz.challenges || [])
        ...
        return handleErr(
          null,
          'CA returned an authorization in an unexpected state' + authz.detail,
          authz
        );
      }
    }
      ...
      request(
        ...
        function(err, res, body) {
          ...
          if (res.statusCode !== 200) {
            return handleErr(
              err,
              'Failed to fetch cert from ' + certUrl,
              res.body.toString()
            );
          }
          ...
          if (body.toString() !== state.certificate.toString()) {
            return handleErr(
              null,
              'Cert at ' + certUrl + ' did not match returned cert'
            );
          }
    ...
    if (
      -1 === ['http-01', 'tls-sni-01', 'dns-01'].indexOf(options.challengeType)
    ) {
      return handleErr(
        new Error(
    ...
    if (!options.newAuthzUrl) {
      return handleErr(
        new Error('options.newAuthzUrl must be the authorization url')
      );
    }
    if (!options.newCertUrl) {
      return handleErr(
        new Error('options.newCertUrl must be the new certificate url')
      );
    }
    if (!options.accountKeypair) {
      if (!options.accountPrivateKeyPem) {
        return handleErr(
          new Error(
            'options.accountKeypair must be an object with `privateKeyPem` and/or `privateKeyJwk`'
          )
        );
      }
      console.warn(
        "'accountPrivateKeyPem' is deprecated. Use options.accountKeypair.privateKeyPem instead."
      );
      options.accountKeypair = RSA.import({
        privateKeyPem: options.accountPrivateKeyPem,
      });
    }
    if (!options.domainKeypair) {
      if (!options.domainPrivateKeyPem) {
        return handleErr(
          new Error(
            'options.domainKeypair must be an object with `privateKeyPem` and/or `privateKeyJwk`'
          )
        );
      }
      console.warn(
        "'domainPrivateKeyPem' is deprecated. Use options.domainKeypair.privateKeyPem instead."
      );
      options.domainKeypair = RSA.import({
        privateKeyPem: options.domainPrivateKeyPem,
      });
    }
    if (!options.setChallenge) {
      return handleErr(
        new Error(
          'options.setChallenge must be function(hostname, challengeKey, tokenValue, done) {}'
        )
      );
    }
    if (!options.removeChallenge) {
      return handleErr(
        new Error(
          'options.removeChallenge must be function(hostname, challengeKey, done) {}'
        )
      );
    }
    if (!(options.domains && options.domains.length)) {
      return handleErr(
        new Error(
          "options.domains must be an array of domains such as ['example.com', 'www.example.com']"
        )
      );
    }
