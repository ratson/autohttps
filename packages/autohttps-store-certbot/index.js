...
function writeRenewalConfig(args) {
  ...
  var liveDir = args.liveDir ||
    path.join(args.configDir, 'live', args.domains[0]);

  var certPath = args.certPath || pyobj.cert || path.join(liveDir, 'cert.pem');
  var fullchainPath = args.fullchainPath ||
    pyobj.fullchain ||
    path.join(liveDir, 'fullchain.pem');
  var chainPath = args.chainPath ||
    pyobj.chain ||
    path.join(liveDir, 'chain.pem');
  var privkeyPath = args.privkeyPath ||
    pyobj.privkey ||
  ...
  var updates = {
    ...
    checkpoints: pyobj.checkpoints, // TODO XXX end
...
var defaults = {
  ...
  privkeyPath: ':configDir/live/:hostname/privkey.pem'
    .split(/\//)
    .join(path.sep),
  fullchainPath: [':configDir', 'live', ':hostname', 'fullchain.pem'].join(
    path.sep
  ),
  ...
  var store = {
    ...
    keypairs: {
      checkAsync: function(keypath, format) {
        ...
        return fs.readFileAsync(keypath, 'ascii').then(
          function(key) {
            if ('jwk' === format) {
    ...
    certificates: {
      ...
      checkAsync: function(args) {
        if (
          ...
          return PromiseA.reject(
            new Error(
              'missing one or more of privkeyPath, fullchainPath, certPath, chainPath from options'
            )
          );
        }
      ...
      setAsync: function(args) {
        return store.configs.getAsync(args).then(function(pyobj) {
          ...
          var archiveDir = args.archiveDir ||
            path.join(args.configDir, 'archive', args.domains[0]);

          var checkpoints = pyobj.checkpoints.toString();
          var certArchive = path.join(
            archiveDir,
            'cert' + checkpoints + '.pem'
          );
          var fullchainArchive = path.join(
            archiveDir,
            'fullchain' + checkpoints + '.pem'
          );
          var chainArchive = path.join(
            archiveDir,
            'chain' + checkpoints + '.pem'
          );
          var privkeyArchive = path.join(
            archiveDir,
            'privkey' + checkpoints + '.pem'
          );

          return mkdirpAsync(archiveDir)
            .then(function() {
              return PromiseA.all([
    ...
    accounts: {
      ...
      _getAccountKeyPath: function(args) {
        ...
        return promise.then(function(accountId) {
          ...
          return args.accountKeyPath ||
            path.join(args.accountsDir, accountId, 'private_key.json');
        });
      },
      // Accounts
          ...
          .then(function(nodes) {
            ...
            return PromiseA.all(
              nodes.map(function(node) {
                return fs
                  .readFileAsync(
                    path.join(args.accountsDir, node, 'regr.json'),
                    'utf8'
                  )
                  .then(function(text) {
      ...
      checkKeypairAsync: function(args) {
        if (!(args.accountKeyPath || args.accountsDir)) {
          return PromiseA.reject(
            new Error(
              'must provide one of options.accountKeyPath or options.accountsDir'
            )
          );
        }
      ...
      checkAsync: function(args) {
        ...
        if (args.accountId) {
          ...
          promise = PromiseA.reject(
            new Error('must provide accountId or email')
          );
        }

        return promise
          .then(function(_accountId) {
            ...
            return PromiseA.all(
              configs.map(function(filename) {
                ...
                return fs
                  .readFileAsync(path.join(accountDir, filename), 'utf8')
                  .then(
                    function(text) {
                      var data;
          ...
          .then(function(hasAccount) {
            ...
            if (
              !Object.keys(files).every(function(key) {
              ...
              err = new Error(
                "Account '" +
                  accountId +
                  "' was corrupt (had id, but was missing files)."
              );
        ...
        return mkdirpAsync(accountDir)
          .then(function() {
            ...
            return PromiseA.all([
              ...
              fs.writeFileAsync(
                path.join(accountDir, 'private_key.json'),
                JSON.stringify(reg.keypair.privateKeyJwk),
                'utf8'
              ),
              // regr.json:
              /*
             ...
             */
              fs.writeFileAsync(
                path.join(accountDir, 'regr.json'),
                JSON.stringify({ body: reg.receipt }),
                'utf8'
              ),
            ]);
          })
      ...
      getAccountIdAsync: function(args) {
        ...
        return pyconf.readFileAsync(args.renewalPath).then(
          function(renewal) {
    ...
    configs: {
      ...
      _checkHelperAsync: function(args) {
        ...
        return pyconf.readFileAsync(args.renewalPath).then(
          ...
          function() {
            return pyconf
              .readFileAsync(path.join(__dirname, 'renewal.conf.tpl'))
              .then(function(pyobj) {
                return pyobj;
              });
          }
      ...
      getAsync: function(args) {
        return store.configs._checkHelperAsync(args).then(function(pyobj) {
          ...
          args.domainKeyPath = args.domainKeyPath ||
            args.keyPath ||
            pyobj.keyPath;
