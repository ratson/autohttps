...
module.exports.create = function(le) {
  ...
  return function(_app) {
    if (_app && 'function' !== typeof _app) {
      throw new Error(
        'use le.middleware() or le.middleware(function (req, res) {})'
      );
    }
    ...
    return function(req, res, next) {
      if (0 !== req.url.indexOf(prefix)) {
        ...
        if ('function' === typeof _app) {
          ...
          res.statusCode = 500;
          res.end(
            "[500] Developer Error: app.use('/', le.middleware()) or le.middleware(app)"
          );
        }
        return;
      }
      ...
      le.challenges[
        'http-01'
      ].get(copy, hostname, token, function(err, secret) {
        if (err || !token) {
          res.statusCode = 404;
