  ...
  var core = {
    ...
    accounts: {
      ...
      registerAsync: function(args) {
        ...
        if (
          !args.email || disagreeTos || parseInt(args.rsaKeySize, 10) < 2048
        ) {
          err = new Error(
          ...
          var promise = le.store.accounts
            ...
            .then(function(keypair) {
              ...
              if (args.accountKeypair) {
                return le.store.accounts.setKeypairAsync(
                  args,
                  RSA.import(args.accountKeypair)
                );
              }
    ...
    certificates: {
      ...
      registerAsync: function(args) {
        var err;
        var challengeDefaults = le[
          '_challengeOpts_' + (args.challengeType || le.challengeType)
        ] || {};
        ...
        return core.accounts.getAsync(copy).then(function(account) {
          ...
          var promise = le.store.certificates
            .checkKeypairAsync(args)
            .then(function(keypair) {
              if (keypair) {
                return RSA.import(keypair);
              ...
              if (args.domainKeypair) {
                return le.store.certificates.setKeypairAsync(
                  args,
                  RSA.import(args.domainKeypair)
                );
              }
            ...
            .then(function(domainKeypair) {
              ...
              return core.getAcmeUrlsAsync(args).then(function(urls) {
                ...
                certReq.setChallenge = function(domain, key, value, done) {
                  ...
                  le.challenges[args.challengeType].set(
                    copy,
                    domain,
                    key,
                    value,
                ...
                certReq.removeChallenge = function(domain, key, done) {
                  log(
                    args.debug,
                    "removeChallenge called for '" + domain + "'"
                  );
                  var copy = utils.merge({ domains: [domain] }, le);
                  utils.tplCopy(copy);

                  le.challenges[args.challengeType].remove(
                    copy,
                    domain,
                    key,
                    done
                ...
                log(
                  args.debug,
                  'calling le.acme.getCertificateAsync',
                  certReq.domains
                );
